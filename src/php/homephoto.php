...
if($method == "hot"):
    try {
        ...
        $sql = "SELECT PATH,ImageID,Title,Description FROM travelimage WHERE ImageID IN (SELECT a.ImageID FROM (SELECT ImageID FROM travelimagefavor GROUP BY ImageID ORDER BY COUNT(*) DESC LIMIT 8) AS a);" ;
        ...
        for($i = $arraynum + 1 ; $i <= 8 ;$i = $i + 1):
            ...
            $sql = "SELECT ImageID,PATH,Title,Description FROM travelimage ORDER BY RAND() LIMIT 1 ;" ;
            $result = $pdo ->query($sql) ;
            $row = $result -> fetch() ;
